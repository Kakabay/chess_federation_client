...
const ContactForm = () => {
  const [contact, setContact]: [Contact[], React.Dispatch<Contact[]>] = useState([
    {
      id: -1,
      phone: '',
      address: '',
  ...
  useEffect(() => {
    if (data.name.length !== 0 && validateEmail(data.email) && data.message.length !== 0) {
      setValidation(true);
    } else {
      setValidation(false);
    }
          ...
          <div className="contact-form-link links">
            <div className="contact-form-link-right">
              {contact[0].id !== -1 ? (
                ...
                <Skeleton height={'3rem'} width={'3rem'} highlightColor={highlightColor} />
              )}
              {contact[0].id !== -1 ? (
                <a href={contact[0].twitter} target="_blank" rel="noreferrer" className="link-icon">
                  <img src={twitter} alt="" />
                </a>
              ) : (
                <Skeleton height={'3rem'} width={'3rem'} highlightColor={highlightColor} />
            ...
            <div className="contact-form-link-right">
              {contact[0].id !== -1 ? (
                <a href={contact[0].youtube} target="_blank" rel="noreferrer" className="link-icon">
                  <img src={yt} alt="" />
                </a>
              ) : (
                <Skeleton height={'3rem'} width={'3rem'} highlightColor={highlightColor} />
        ...
        <div className="contact-form-right-bottom">
          <div className="input-blocks-wrapper">
            <div className="input-block">
              ...
              <span
                className={
                  alert.name && data.name.length === 0 ? 'form-warning active' : 'form-warning'
                }>
            ...
            <div className="input-block">
              ...
              <span
                className={
                  alert.email && !validateEmail(data.email) ? 'form-warning active' : 'form-warning'
                }>
          ...
          <span className="form-note">
            Поля отмеченные <span className="red">*</span> обязательны для заполнения
          </span>
